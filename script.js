...
function loadLanguages() {
  ...
  state.allVoices.forEach((voice) => {
    ...
    if (langTag in config.LANGUAGE_NAMES) {
      uniqueLanguages.set(langTag, config.LANGUAGE_NAMES[langTag]);
    }
  });
  const sortedLanguages =
      Array.from(uniqueLanguages).sort((a, b) => a[1].localeCompare(b[1]));

  const currentLang = ui.languageSelector.value;
  ui.languageSelector.innerHTML = '';
  sortedLanguages.forEach(([langTag, displayName]) => {
    const option = new Option(displayName, langTag);
    ui.languageSelector.add(option);
  });

  const currentLangExists =
      sortedLanguages.some((lang) => lang[0] === currentLang);
  if (currentLangExists) {
    ui.languageSelector.value = currentLang;
  } else {
    const defaultLangExists =
        sortedLanguages.some((lang) => lang[0] === config.DEFAULT_LANGUAGE);
    ui.languageSelector.value =
        defaultLangExists ?
            config.DEFAULT_LANGUAGE : (sortedLanguages[0]?.[0] || '');
  }
...
function updateVoiceList() {
  ...
  const filteredVoices =
      state.allVoices.filter((voice) => voice.lang === selectedLang);
  const sortedVoices =
